...
fn main() {
    ...
    timely::execute_from_args(std::env::args(), |worker| {
        ...
        let exchange = Exchange::new(move |update: &((String,()),usize,i32)| (update.0).0.hashed().into());

        // create a new input, exchange data, and inspect its output
        worker.dataflow::<usize,_,_>(|scope| {
            let collection: CollectionCore<_, Vec<_>, _, _> = input.to_stream(scope)
                .as_collection();
        ...
        for _ in 0..batch_count {
            ...
            for _ in 0..size {
                ...
                stack.push(((String::from_iter((0..len).map(|_| rng.gen_range('a'..'c'))), ()), 1, 1));
            }
            batches.push(stack);
        }
        println!("Done");
